#!/usr/bin/env python3
"""
Alien Shooter Game - Version 0.4
This is the active development version of the game.
"""
...
# Add global font cache to prevent memory leaks from repeated font creation
...
class Player(pygame.sprite.Sprite):
    def __init__(self, game):
        ...
        self.speed = 8
        self.health = 100
        self.stored_medkits = 5  # Start with 5 medkits
        self.max_medkits = 10  # Increase max medkits to 10
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, x, y, target_x, target_y, weapon_type, damage):
        ...
        # Add lifetime to ensure bullets despawn
        self.creation_time = pygame.time.get_ticks()
        self.lifetime = 5000  # 5 seconds max lifetime
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, x, y, enemy_type="standard"):
        ...
        # Enemy type determines stats and appearance
        ...
        # Proximity avoidance parameters
        self.avoidance_radius = 40  # Distance at which to start avoiding other enemies
        self.avoidance_force = 0.5  # Strength of the avoidance force (0.0 to 1.0)
    ...
    def update(self, player, game, camera_x=0, camera_y=0):
        ...
        # Check if enemy is visible on screen with a buffer
        is_visible = game.is_sprite_visible(self, buffer=100)
        
        # Store old position for collision detection
        old_x = self.rect.x
        old_y = self.rect.y
        
        # Tiered update system based on distance and visibility
        update_tier = 0  # Full updates for visible enemies
        if not is_visible:
            if distance_to_player > 1500:
                update_tier = 2  # Minimal updates: direct movement only
            elif distance_to_player > 800:
                update_tier = 1  # Medium updates: simplified pathfinding
        
        # Skip some updates for distant enemies (tier 2)
        if update_tier == 2 and (current_time // 250) % 4 != (id(self) % 4):
            return  # Only update every 4th frame based on object id
        
        # Handle shooting behavior for shooter type and visible enemies
        if self.type == "shooter" and is_visible:
            if distance_to_player <= self.max_fire_range and current_time - self.last_shot >= self.shot_delay:
                # Calculate angle to player
                self.angle = math.atan2(dy, dx)
                # Check line of sight only for visible enemies
                if not game.map.check_line_of_sight(self.rect.centerx, self.rect.centery,
                                              player.rect.centerx, player.rect.centery):
                    self.shoot(game)
                    self.last_shot = current_time
        
        # Handle melee attacks for non-shooter types that are close
        ...
        # For off-screen enemies, use simplified movement
        if update_tier >= 1:
            # Skip pathfinding and expensive AI operations
            # Move directly towards player with simplified collision
            move_speed = self.speed * (0.7 if update_tier == 2 else 0.9)
            
            if distance_to_player > 0:
                # Direct movement vector
                move_x = (dx / distance_to_player) * move_speed
                move_y = (dy / distance_to_player) * move_speed
                
                # Apply minimal or no avoidance for off-screen enemies
                if update_tier == 1 and random.random() < 0.3:  # Only occasionally check proximity
                    move_x, move_y = self.apply_simplified_avoidance(game, move_x, move_y)
                
                # For tier 2, just move directly without collision checks
                if update_tier == 2:
                    self.rect.x += move_x
                    self.rect.y += move_y
                else:
                    # For tier 1, use simplified collision (only check horizontal OR vertical)
                    self.rect.x += move_x
                    if game.map.check_collision(self):
                        self.rect.x = old_x
                        self.rect.y += move_y
            ...
            # For visible enemies, use more advanced pathfinding but at reduced frequency
            should_update_path = (
            ...
            if self.path and self.path_index < len(self.path):
                ...
                if dist < PATH_SMOOTHING_DISTANCE:
                    ...
                    # Apply full proximity avoidance for visible enemies
                    move_x, move_y = self.apply_proximity_avoidance(game, move_x, move_y)
                    
                    # Full collision checking for visible enemies
                    # Try moving horizontally first
                    self.rect.x += move_x
                    if game.map.check_collision(self):
                        self.rect.x = old_x
        ...
        # Only check for stuck state for visible enemies
        if is_visible:
    ...
    def apply_proximity_avoidance(self, game, move_x, move_y):
        """Apply small adjustments to movement to avoid overlapping with other enemies"""
        # Only apply avoidance if we're moving
        if move_x == 0 and move_y == 0:
            return move_x, move_y
            
        # Get nearby sprites using the quadrant system
        nearby_sprites = game.get_nearby_sprites(self, self.avoidance_radius * 2)
        
        # Filter to only include other enemies
        nearby_enemies = [sprite for sprite in nearby_sprites 
                          if sprite in game.enemies and sprite != self]
        ...
        for enemy in nearby_enemies:
            # Calculate distance and direction to the other enemy
            ...
            if distance < self.avoidance_radius and distance > 0:
                ...
                factor = 1.0 - (distance / self.avoidance_radius)  # Closer means stronger avoidance
                avoid_x += dx / distance * factor
                avoid_y += dy / distance * factor
                avoidance_count += 1
        
        # Apply avoidance if we found enemies to avoid
        if avoidance_count > 0:
            # Normalize the avoidance vector
            length = math.sqrt(avoid_x * avoid_x + avoid_y * avoid_y)
            if length > 0:
                avoid_x = avoid_x / length
                avoid_y = avoid_y / length
                
                # Blend original movement with avoidance
                new_x = move_x * (1 - self.avoidance_force) + avoid_x * self.avoidance_force * self.speed
                new_y = move_y * (1 - self.avoidance_force) + avoid_y * self.avoidance_force * self.speed
                
                # Normalize to keep the same speed
                new_length = math.sqrt(new_x * new_x + new_y * new_y)
                if new_length > 0:
                    speed_factor = math.sqrt(move_x * move_x + move_y * move_y) / new_length
                    return new_x * speed_factor, new_y * speed_factor
        
        return move_x, move_y

    def apply_simplified_avoidance(self, game, move_x, move_y):
        """Apply minimal adjustments to movement to avoid overlapping with other enemies"""
        # Only apply avoidance if we're moving
        if move_x == 0 and move_y == 0:
            return move_x, move_y
...
class Map:
    ...
    def draw_text(self, screen, text, size, color, x, y):
        # Use font cache to prevent memory leaks
...
class Game:
    def __init__(self):
        ...
        # Create persistent UI surfaces to prevent memory leaks from repeated surface creation
        self.hud_overlay = pygame.Surface((SCREEN_WIDTH, 100), pygame.SRCALPHA)
        self.weapon_overlay = pygame.Surface((200, 100), pygame.SRCALPHA)
        self.medkit_overlay = pygame.Surface((200, 30), pygame.SRCALPHA)
        self.minimap_surface = pygame.Surface((150, 150), pygame.SRCALPHA)
    ...
    def cleanup_off_screen_sprites(self):
        current_time = pygame.time.get_ticks()
        if current_time - self.last_cleanup > 250:  # Check more frequently (250ms)
            # Clean up bullets that are off screen or too far from player
            for bullet in list(self.bullets):  # Convert to list to avoid modification during iteration
                # Check both visibility and distance from player
                too_far = False
                if hasattr(bullet, 'rect') and hasattr(self.player, 'rect'):
                    dx = bullet.rect.centerx - self.player.rect.centerx
                    dy = bullet.rect.centery - self.player.rect.centery
                    distance_sq = dx*dx + dy*dy
                    too_far = distance_sq > 1000000  # ~1000 pixels
                
                if too_far or not self.is_sprite_visible(bullet, buffer=500):
                    bullet.kill()
            
            # Clean up enemy bullets
            for bullet in list(self.enemy_bullets):
                # Same distance and visibility check
                too_far = False
                if hasattr(bullet, 'rect') and hasattr(self.player, 'rect'):
                    dx = bullet.rect.centerx - self.player.rect.centerx
                    dy = bullet.rect.centery - self.player.rect.centery
    ...
    def draw_pause_menu(self):
        # No need to create a semi-transparent overlay - that's now handled in the draw method
    ...
    def draw_minimap(self):
        # Reuse minimap surface to prevent memory leaks
        ...
        pygame.draw.rect(self.minimap_surface, (0, 0, 0, 128), (0, 0, minimap_size, minimap_size))
        ...
        pygame.draw.circle(self.minimap_surface, YELLOW, (int(player_mini_x), int(player_mini_y)), 3)
        ...
        for enemy in self.enemies:
            if (world_x - 100 <= enemy.rect.centerx <= world_x + 2100 and
                ...
                pygame.draw.circle(self.minimap_surface, RED, (int(enemy_mini_x), int(enemy_mini_y)), 2)
        
        # Draw minimap border
        pygame.draw.rect(self.minimap_surface, WHITE, (0, 0, minimap_size, minimap_size), 1)
    ...
    def draw(self):
        ...
        if self.state == MENU:
            ...
            # Clear and prepare HUD overlay
            self.hud_overlay.fill((0, 0, 0, 0))  # Clear with transparent black
            pygame.draw.rect(self.hud_overlay, (0, 0, 0, 128), (0, 0, SCREEN_WIDTH, 100))
            self.screen.blit(self.hud_overlay, (0, 0))
            ...
            # Clear and prepare weapon info overlay
            self.weapon_overlay.fill((0, 0, 0, 0))  # Clear with transparent black
            pygame.draw.rect(self.weapon_overlay, (0, 0, 0, 128), (0, 0, 200, 100))
            self.screen.blit(self.weapon_overlay, (0, SCREEN_HEIGHT - 100))
            ...
            # Clear and prepare medkit overlay
            self.medkit_overlay.fill((0, 0, 0, 0))  # Clear with transparent black
            pygame.draw.rect(self.medkit_overlay, (0, 0, 0, 128), (0, 0, 200, 30))
            self.screen.blit(self.medkit_overlay, (0, SCREEN_HEIGHT - 90))
            ...
            for sprite in self.all_sprites:
                ...
                screen_y = sprite.rect.y - self.camera_y
                self.screen.blit(sprite.image, (screen_x, screen_y))
            # Then draw the pause menu on top using pre-created overlay
