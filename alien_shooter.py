...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption("Hunter")  # Changed game title
    ...
    def spawn_enemy(self, enemy_type):
        ...
        screen_right = self.camera_x + SCREEN_WIDTH
        screen_top = self.camera_y
        screen_bottom = self.camera_y + SCREEN_HEIGHT
        
        # Add buffer zone (100 pixels outside screen)
        buffer = 100
        
        # Choose a random side to spawn (0: top, 1: right, 2: bottom, 3: left)
        side = random.randint(0, 3)
        
        if side == 0:  # Top
            x = random.randint(screen_left - buffer, screen_right + buffer)
            y = screen_top - buffer
        elif side == 1:  # Right
            x = screen_right + buffer
            y = random.randint(screen_top - buffer, screen_bottom + buffer)
        elif side == 2:  # Bottom
            x = random.randint(screen_left - buffer, screen_right + buffer)
            y = screen_bottom + buffer
        else:  # Left
            x = screen_left - buffer
            y = random.randint(screen_top - buffer, screen_bottom + buffer)
        ...
        # Find valid spawn position near the chosen point
        valid_x, valid_y = self.map.find_valid_spawn_position(
            30, 30,
            min_distance=None,
            center_point=(x, y)
        )
        
        enemy = Enemy(valid_x, valid_y, enemy_type)
...
if __name__ == "__main__":
    ...
    if not os.path.exists("map_data.json") or not os.path.exists("demo_map.jpg"):
        print("Generating new map...")
        demo_map = Map(load_saved=False)  # Force new map generation
        print("Map saved as 'demo_map.jpg' and 'map_data.json'")
    else:
